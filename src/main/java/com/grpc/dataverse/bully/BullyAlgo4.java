...
import com.grpc.dataverse.file.server.FileService;
...
public class BullyAlgo4 implements Runnable{
    static int leader_id=-1;
    static int self_id=-1;
    static int server_Port = 5514 ;
    String operation;
    String reqtype;
    static int source_id=-1;
    static HashMap<Integer,String> processes= new HashMap<Integer,String>();
    static HashMap<Integer,Integer> processes_port= new HashMap<Integer,Integer>();
    ...
    static String FOLDER_PATH = "F:\\275\\server4\\";

    public BullyAlgo4(String operation) {
        this.operation = operation;

    }

    public BullyAlgo4(String operation, String reqtype) {
        this.operation = operation;
    ...
    public static void main(String args[]) throws UnknownHostException, IOException, InterruptedException{
        ...
        Server server = ServerBuilder.forPort(55140)
//				.maxInboundMessageSize(FILE_SPLIT_UNIT)
//				.maxInboundMetadataSize(Integer.MAX_VALUE)
                .addService(new FileService4())
                .addService(new MessageService())
                .build();

        server.start();

        Runnable receiver = new BullyAlgo4("receiver");
        new Thread(receiver).start();

        // Thread.sleep(10);


        Runnable heartbeat = new BullyAlgo4("heartbeat");
        ...
        if(operation.equals("timer")){
            ...
            try{
                ...
                if(!received){
                    ...
                    Runnable sender = new BullyAlgo4("sender","cood");
                ...
                if(received && !leader_flag){
                    ...
                    Runnable sender = new BullyAlgo4("sender","elecreq");
                    new Thread(sender).start();
                ...
                while(true){
                    ...
                    if(option.equals("elecreq")){
                        ...
                        if(self_id>source_id){
                            Runnable sender = new BullyAlgo4("sender","ok");
                            new Thread(sender).start();
                        ...
                        if(!election_req){
                            ...
                            Runnable timer = new BullyAlgo4("timer");
                        ...
                        System.out.println("Received OK from "+processes.get(sender)+";"+processes_port.get(sender));
                        ...
                        election_req=false;
                        received=false;
                        System.out.println("******LEADER selected is :"+leader_id+" : "+processes.get(leader_id)+":"+processes_port.get(leader_id)+ " Leader Flag= "+leader_flag);
                        ...
                        int sender=Integer.parseInt(in.readUTF());
                        System.out.println("HEARTBEAT received from "+processes.get(sender)+":"+processes_port.get(sender));
            ...
            while(true){
                try{
                    ...
                    if(leader_flag && (self_id!=leader_id)){
                        ...
                        Socket socket = new Socket("127.0.0.1" , processes_port.get(leader_id));
                        DataOutputStream out = new DataOutputStream(socket.getOutputStream());
                        out.writeUTF("heartbeat");
                        out.writeUTF(self_id+"");

                        System.out.println("Sent HEARTBEAT to : "+"127.0.0.1" +":"+ processes_port.get(leader_id));
    ...
    public static void sendCoordinatorMsg() throws IOException{
        for (int key : processes.keySet()) {
            if(key!=self_id){
                ...
                try{
                    Socket socket = new Socket(destination_server , processes_port.get(key));
                    DataOutputStream out = new DataOutputStream(socket.getOutputStream());
                    out.writeUTF("cood");
                    out.writeUTF(leader_id+"");

                    System.out.println("Sent Leader ID to : "+destination_server+":"+processes_port.get(key));
                }
                catch(Exception e){
                    System.out.println("The process "+destination_server+":"+processes_port.get(key)+" has failed, won't get the new leader !");
                }
    ...
    public static void sendOK() throws IOException{
        try{
            ...
            Socket socket = new Socket(destination_server, processes_port.get(source_id));
            DataOutputStream out = new DataOutputStream(socket.getOutputStream());
            out.writeUTF("ok");
            out.writeUTF(self_id+"");
            System.out.println("Sent OK to : "+destination_server+":"+processes_port.get(source_id));
        }
        catch(Exception e){
            System.out.println("Process "+processes.get(source_id)+":"+processes_port.get(source_id)+" has FAILED. OK Message cannot be sent !");
        }
        ...
        for (int key : processes.keySet()) {
            if(key>self_id){
                ...
                try{
                    ...
                    System.out.println("Sent Election Request to : "+destination_server+":"+processes_port.get(key));
                }
                catch(Exception e){
                    System.out.println("The process :"+destination_server+":"+processes_port.get(key)+" has FAILED, cannot send Election Request !");
                    failure++;
                }
            }
    ...
    public static void initialize() throws UnknownHostException, IOException{
        processes.put(5,"127.0.0.1");
        processes.put(4,"127.0.0.1");
        processes.put(3,"127.0.0.1");
        processes.put(2,"127.0.0.1");
        processes.put(1,"127.0.0.1");



        processes_port.put(5,5511);
        processes_port.put(4,5512);
